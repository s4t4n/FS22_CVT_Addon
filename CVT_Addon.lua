...
function CVTaddon:onRegisterActionEvents()
    ...
    if self.getIsEntered ~= nil and self:getIsEntered() then
		...
		-- D1
        _, spec.eventIdV1 = self:addActionEvent(spec.actionEventsV1, 'SETVARIOONE', self, CVTaddon.VarioOne, false, true, false, true, nil)
		...
		-- D2
		_, spec.eventIdV2 = self:addActionEvent(spec.actionEventsV2, 'SETVARIOTWO', self, CVTaddon.VarioTwo, false, true, false, true, nil)
		...
		-- AR
		_, spec.eventIdV3 = self:addActionEvent(spec.actionEventsV3, 'LMBF_TOGGLE_RAMP', self, CVTaddon.AccRamps, false, true, false, true, nil)
		...
		-- BR
		_, spec.eventIdV4 = self:addActionEvent(spec.actionEventsV4, 'LMBF_TOGGLE_BRAMP', self, CVTaddon.BrakeRamps, false, true, false, true, nil)
		...
		-- neutral
		_, spec.eventIdV5 = self:addActionEvent(spec.actionEventsV5, 'SETVARION', self, CVTaddon.VarioN, false, true, false, true, nil)
		...
		-- rpmUP
		_, spec.eventIdV6 = self:addActionEvent(spec.actionEventsV6, 'SETVARIORPMP', self, CVTaddon.VarioRpmPlus, false, true, false, true, nil)
		...
		-- rpmDn
		_, spec.eventIdV7 = self:addActionEvent(spec.actionEventsV7, 'SETVARIORPMM', self, CVTaddon.VarioRpmMinus, false, true, false, true, nil)
...
function CVTaddon.initSpecialization()
    ...
    -- schemaSavegame:register(XMLValueType.BOOL, "vehicles.vehicle(?).CVTaddon#eventIdV4")

    -- schemaSavegame:register(XMLValueType.INT, "vehicles.vehicle(?).CVTaddon#DamagesAfterInspection")
end
...
function CVTaddon:VarioN() -- neutral
	...
	if (spec.vFour == 0) then
		...
		-- self.spec_motorized.motor.differentialRotSpeed = 0
		-- self.spec_motorized.motor.gearRatio = 0
...
function CVTaddon:onUpdate(dt)
	...
	if isEntered and spec.isVarioTM then
		...
		if spec.vOne ~= 1 and spec.vOne ~= nil and spec.isVarioTM and spec.vFour ~= 0 then
			...
			self.spec_motorized.motor.gearRatio = self.spec_motorized.motor.gearRatio * 1.2
			if spec.spiceRPM >= 1200 and spec.vFive <= 7 then
		...
		if spec.vOne == 1 and spec.vOne ~= nil and spec.isVarioTM and spec.vFour ~= 0 then
			...
			if spec.spiceRPM >= 1100 and spec.vFive <= 7 then
	...
	-- -- Dev Text Help Zeug -------------------------------------------------------------------------------------------------------------------------------
			...
			-- g_currentMission:addExtraPrintText("spiceLoad: " .. tostring(spec.spiceLoad))
	--------------------------------------------------------------------------------------------------------------------------------------------------------------
	end
end
